...
async function dagrapport(datum?: string) {
  ...
  const html = `<h2 style="margin:0 0 4px;font-size:20px">Dagrapport ${fmt(day)}</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:13px">${rs.sort((a, b) => (a.dienst + a.route).localeCompare(b.dienst + b.route)).map(r => `<tr style="border-bottom:1px solid #E4E8EF"><td style="padding:6px 8px 6px 0">${esc(r.dienst)}</td><td style="padding:6px 8px 6px 0"><b>${esc(r.route)}</b></td><td style="padding:6px 8px 6px 0">${esc(r.medewerker || "–")}</td><td style="padding:6px 8px 6px 0">${esc(r.gehaald)}/${esc(r.doel)}${r.laatste_scan ? `<br><span style="color:#9AA3B0;font-size:11px">laatste ${esc(r.laatste_scan)}</span>` : ""}</td><td style="padding:6px 0">${scoreChip(pct(r))}</td><td style="padding:6px 0;color:#6B7280">${esc(r.reden || "")}${r.status === "Te bevestigen" ? chip("niet bevestigd", "#FBF1DC", "#B7791F") : ""}</td></tr>`).join("") || `<tr><td style="padding:6px 0;color:#6B7280">Geen dienstrapporten voor deze dag.</td></tr>`}</table>
    ${onder.length ? `<h3 style="font-size:14px;margin:18px 0 8px;color:#C62828">Onder de norm (${onder.length})</h3>${onder.map(rapportHtml).join('<hr style="border:0;border-top:1px solid #E4E8EF;margin:10px 0">')}` : ""}
